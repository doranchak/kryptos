...
void encrypt(char *plaintext, int plaintext_length, char *keyword, char *cycleword) {
    ...
    for (int i=0; i<(int)plaintext_length; i++) {
        // char pt_ch = file_contents[i];
        int pt = plaintext[i] - 'A';
        int pos = (index[pt] + index[cycleword[i%len]-'A']) % ALPHABET_SIZE;
        char ct = alphabet[pos] + 'A';
        printf("%c", ct);
    }
    printf("\n");
}
...
int main(int argc, char *argv[]) {
    if (argc != 4) {
        fprintf(stderr, "Encrypts the given plaintext with Quagmire III using the given keyword and cycle (indicator) word.\n");
        fprintf(stderr, "Usage: %s <plaintextext_file> <keyword> <cycleword>\n", argv[0]);
        return 1;
    }
    ...
    encrypt(file_contents, (int)j, argv[2], argv[3]);
    return 0;
}
